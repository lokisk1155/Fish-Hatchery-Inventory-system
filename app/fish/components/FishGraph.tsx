...
export default function FishGraph({ fishIndexEntryList }: Props) {
  ...
  return (
    <div style={{ marginBottom: '100px' }} className="mt-8 flex flex-col items-center space-y-4">
          ...
          <div key={index} className="flex flex-col items-start mx-2 space-y-2">
            <div className="relative h-5" style={{ width: `${containerWidth}px` }}>
              <div
                ...
                title={`Date: ${new Date(fish.date_caught).toISOString().split('T')[0]}, Length: ${
                  fish.total_length
                }`}
                style={{
            ...
            <div className="relative h-5" style={{ width: `${containerWidth}px` }}>
              <div
                ...
                title={`Date: ${new Date(fish.date_caught).toISOString().split('T')[0]}, Weight: ${
                  fish.weight
                }`}
                style={{
            ...
            <div className="text-xs">{new Date(fish.date_caught).toISOString().split('T')[0]}</div>
          </div>
        ))}
      </div>
